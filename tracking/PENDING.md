...
## 🟡 Media Prioridad
- [ ] **Motor del Lab: keywords que no aplica según canon 1.0.2** (Lead Developer, espera decisión de Marcos; cambia resultados del Lab): (1) FEAR: el motor hace una tirada ~30% que anula la carga; canon = -1 DICE a los ataques cuerpo a cuerpo contra el modelo, y los FEAR son inmunes entre sí. (2) TOUGH: el motor aplica -1 INJURY DICE siempre; canon = el primer Out of Action pasa a Down. (3) FIRE / GAS / SHRAPNEL: el motor no pone el BLOOD MARKER extra tras la Injury Roll, y NEGATE FIRE/GAS lo modela como -1 INJURY DICE (eso es Protection From Harm de la Alchemist Armour, no NEGATE). (4) DEADLY: el motor añade 1 BLOOD MARKER; canon 1.0.2 = Injury Roll con 3D6. (5) CLEAVE (X): solo puntúa al elegir arma; canon = X ataques cuerpo a cuerpo. (6) RELOAD: no termina la activación. (7) FLAMETHROWER: el motor alcanza 1-2 modelos extra en línea; canon = Success automático sin Critical, sin objetivos extra. Fuente: `KEYWORD_GLOSSARY` / `Glosario-keywords-1.0.2.pdf`.
